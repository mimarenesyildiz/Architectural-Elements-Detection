    ...
    def load_mask(self, image_id):
       ...
       Returns:
        ...
        for i, p in enumerate(annotations):
            try:
                rr, cc = skimage.draw.polygon(p.get('all_points_y', []), p.get('all_points_x', []))
                masks[:, :, i] = self.draw_mask(rr, cc, height, width)
                class_ids[i] = p['class_ids']
            except KeyError as e:
                print(f"KeyError: {e} in image {image_id}, skipping annotation.")
